...
def SaveDecoderAndData(clf, X, y, subjID, featureType, error_template, correct_template, model_type):
    """
    ...
    if model_type in ['original','Original','orig','ORIGINAL']:
        model_file = 'Models/' + subjID + '/Session1/' + subjID + '_Error_classifier_' + time_to_save[:19] + '.sav'
    else:
        model_file = 'Models/' + subjID + '/' + model_type + '/Session1/' + subjID + '_Error_classifier_' + time_to_save[:19] + '.sav'
    pickle.dump(model, open(model_file, 'wb'))
    
    if model_type in ['original','Original','orig','ORIGINAL']:
        filepath_export_data = 'Models/' + subjID + '/Session1/' + subjID + '_data_for_Error_classifier_' + time_to_save[:19] + '.npz'
    else:
        filepath_export_data = 'Models/' + subjID + '/' + model_type + '/Session1/' + subjID + '_data_for_Error_classifier_' + time_to_save[:19] + '.npz'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f','--findPaths',help='Manually enter paths? (should always be n or no when using arguments)', required=False)
    ...
    if arguments['findPaths'] is None:
        findPaths = input('Manually enter in file paths (y/n)?: ')
    else:
        findPaths = arguments['findPaths']
    ...
    if findPaths in ['y', 'Y', 'yes', 'Yes', 'YES']:
        ...
        eeg_files = glob.glob('SaveData/' + subjID + '/Session1/*' + subjID + '_Error*.easy')
        filename_eeg = eeg_files[-1] # load the most recent eeg file
        print(filename_eeg)

        behav_files = glob.glob('SaveData/' + subjID + '/Session1/Error_Screening_' + subjID + '*.csv')
    ...
    SaveDecoderAndData(clf, X, y, subjID, featureType, error_template, correct_template, model_type)