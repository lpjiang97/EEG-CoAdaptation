...
class MyGame(arcade.Window):
    ...
    def __init__(self, width, height, taskData, col_names, EEGdevice, outlet, C3, C4, clf, X_loaded):
        ...
        self.C3 = C3 # channel number for EEG electrode C3
        self.C4 = C4 # channel number for EEG electrode C4
        self.nyq = 0.5 * self.fs

        # Store the classifier/model and it's associated data for scaling
        self.clf = clf
    ...
    def classify_epoch(self):
        ...
        # Filter our data
        #print('data (pre-filter): ' + str(self.eeg_data))
        filtered = filter_data(self.eeg_data.T, sfreq=self.fs, l_freq=7, h_freq=31)
        data = filtered.T
        ...
        for chanOfInt in channels:
            ...
            #print('X[' + str(cnt) + ']: ' + str(X[cnt]))
            #print('f: ' + str(f))
            #print('alpha_idx: ' + str(alpha_idx))
            #print('Pxx_den[alpha_idx]: ' + str(Pxx_den[alpha_idx]))
            cnt += 1
        ...
        print('X: ' + str(X))
        X = np.asarray(X)
        X = (X - np.mean(self.X_loaded,0))/np.std(self.X_loaded,0)
        X = X.reshape(1,-1)
...
def saveData(subjID, taskParameters, taskData):
    ...
    fileNum = len(glob.glob('SaveData/BCI_' + subjID + '*.csv')) + 1

    # Save the task parameters to json file
    with open('SaveData/BCI_' + subjID + '_R' + str(fileNum) + '_Parameters.json', 'w') as fp:
        json.dump(taskParameters, fp)

    # Save the data to csv file
    file_name = ('SaveData/BCI_' + subjID + '_R' + str(fileNum) + '.csv')
    taskData.to_csv(file_name, encoding='utf-8')

    # Save the data as a json file
    taskData.to_json('SaveData/BCI_' + subjID + '_R' + str(fileNum) + '.json')
...
if __name__ == "__main__":
    ...
    if EEGdevice == 7:
        ...
        print('Make sure C3 is channel 3, and C4 is channel 4')
        C3 = 3
        C4 = 4
        outlet = []
    elif EEGdevice == 8:
        print('Enobio selected')
        C3 = int(input('Enter channel number for C3 (first channel is channel 0): '))
        C4 = int(input('Enter channel number for C4 (first channel is channel 0): '))
    ...
    game = MyGame(SCREEN_WIDTH, SCREEN_HEIGHT, taskData, col_names, EEGdevice, outlet, C3, C4, clf, X_loaded)
    game.setup()
    arcade.run()
