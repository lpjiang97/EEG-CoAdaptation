...
def runTrial(subjID, trialType, taskData, win, clock, tone, EEGdevice, outlet):
    ...
    if trialType == 'L':
        stimulus = 'left'
        img = visual.ImageStim(win=win, image='Images/arrow_left.png', units='pix')
    elif trialType == 'R':
        stimulus = 'right'
        img = visual.ImageStim(win=win, image='Images/arrow_right.png', units='pix')
    ...
    # Record for 5 seconds before moving on
    trialLength = 4
    core.wait(trialLength)
...
if __name__ == "__main__":
    ...
    message = visual.TextStim(win, text='If you see a left arrow, imagine moving your left hand until you see the +')
    message.setAutoDraw(True)  # automatically draw every frame
    win.flip()
    core.wait(4)
    message.setText('If you see a right arrow, imagine moving your right hand until you see the +')
    win.flip()
    core.wait(4)
    message.setText('When you see the +, relax')
    win.flip()
    core.wait(3)
